...
public class LoginGoogleServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (code == null || code.isEmpty()) {
            ...
            try {
                User user = new UserGet().getUser(Long.parseLong(googleAccount.getId()));
                if (user.getUserID() == 0) {
                    user = new User(Long.parseLong(googleAccount.getId()), googleAccount.getName(), googleAccount.getEmail(), "", false, "");
                    new UserGet().insertUser(user);
                } else {
